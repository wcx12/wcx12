...
test('generated code blocks and article contents remain keyboard reachable', async () => {
  ...
  for (const file of articleFiles) {
    ...
    if (file.includes('tiger-generative-retrieval-reading')) {
      ...
      assert.match(generatorFigure, /class="tiger-transformer-tower tiger-transformer-decoder"/);
      assert.equal((generatorFigure.match(/class="tiger-transformer-layer"/g) ?? []).length, 8);
      assert.equal((generatorFigure.match(/class="tiger-transformer-op tiger-transformer-attn"/g) ?? []).length, 8);
      assert.equal((generatorFigure.match(/class="tiger-transformer-op tiger-transformer-cross"/g) ?? []).length, 4);
      assert.match(generatorFigure, /input 128[\s\S]*?6 heads x 64[\s\S]*?MLP 1024/);
      assert.match(generatorFigure, /P\(d1 \| context\)[\s\S]*?P\(d4 \| context, d1,d2,d3\)/);
