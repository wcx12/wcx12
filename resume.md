# Academic Profile

## Education

- **Beijing Institute of Technology**\
  Undergraduate study, Sep 2022-Jun 2026

## Experience

- **Shenzhen Houlang Pengpai Technology Co., Ltd.**\
  Entrepreneurial builder, Mar 2026-present; early-stage education-technology practice, Shenzhen

## Publications

{{PUBLICATIONS}}

## Research Interests

- **3D Geometry**
  - **[Point-set Registration](../research/point-cloud-registration/)**\
    Robust alignment under noisy, partial, and imperfect observations
- **Computer Vision**
  - **[Visual Place Recognition](../research/vpr/)**\
    Retrieval and localization-oriented benchmark research
  - **[Medical Image Analysis](../research/medical-image-analysis/)**\
    Active learning and sample-efficient image classification
- **Large Models**
  - **[Agent Systems](../research/agent/)**\
    Retrieval, tool use, planning, and evaluation workflows
- **AI for Education**
  - **[Learning Tools](../research/ai4edu/)**\
    Educational software and interactive mathematics resources

## Selected Projects

- **[FusionTrack](https://github.com/wcx12/FusionTrack)**\
  Graduation thesis and point-set registration artifacts with auditable benchmark protocols and reproducible evaluation tooling
- **[major-intel](https://github.com/wcx12/major-intel)**\
  A MySQL-backed, SQL-first retrieval agent with tool traces and explicit evidence-gap handling
- **[shuxuepeiyou](https://github.com/wcx12/shuxuepeiyou)**\
  LaTeX lecture notes, published PDFs, and interactive visualizations for mathematics enrichment

## Technical Skills

- **Research stack**\
  Python, PyTorch, TensorFlow, OpenCV, and Scikit-learn
- **Retrieval and data systems**\
  MySQL, SQL-first retrieval, function-calling agents, and evidence tracking
- **Reproducible experimentation**\
  Linux, Git, benchmark protocols, strict evaluation audits, and test automation
- **Scientific publishing**\
  LaTeX, GitHub Actions, technical documentation, and interactive visualization
