...
编号 233 和 234 相邻，并不意味着对应的两个物品相似。如果每个物品对应输出词表中的一个独立类别，让 Transformer 预测 Item ID 就接近一个超大规模分类问题。低频物品的专属表示缺少训练信号，训练词表之外的新物品也不能直接作为已有类别输出。这是原子 ID 配置的限制，并不是所有生成式编号方案都必须面对同样的词表问题。
...
这个变化改变的是物品表示的组织方式。普通 Item ID 的编号本身不编码内容相似性，但模型仍然可以从交互中学习物品之间的关系；Semantic ID 则将部分内容关联直接带进编号结构，让不同物品共享某些位置的 token。模型在高频物品上学到的这些 token 表示，也有机会被共享相同编号的低频物品使用。
...
TIGER 主实验采用 3 层残差码本，每层码本包含 256 个码向量（codeword）；每层选中一个向量并记录它的整数编号，因此 RQ-VAE 先为每个 item 生成三位 Semantic ID。下文用“码向量”指向量，用“码字编号”指对应的整数。
...
图 2 的重构路径已经得到输出 $\hat x$。训练时，先比较它与原始内容 embedding $x$ 的差异，得到重构损失：
...
其中，$L_d$ 是第 $d$ 层的量化损失，$r_d$ 是该层残差，$e_{c_d}$ 是该层被选中的码向量，$\beta$ 是 commitment 项权重；$\operatorname{sg}$ 表示 stop-gradient：前向计算时保留原值，反向传播时梯度为零。

两项都度量残差与所选码向量的距离，但梯度方向不同：第一项把残差当作目标来更新码向量，第二项把码向量当作目标来约束残差。stop-gradient 决定每一项中哪一侧接收梯度，$\beta$ 则控制第二项的强度。
...
所以这一项的作用是训练码本：哪个码向量被选中了，就把它拉向当前残差，让码本逐渐贴近数据分布。
...
如果码本随机初始化，一些码向量可能远离训练样本。一个码向量如果从未被最近邻选择命中，就缺少选中样本带来的训练信号，成为不活跃码字（dead code）。大量样本集中选择少数码向量，就是码本坍缩（codebook collapse）的典型表现。

论文在第一个训练 batch 上做 K-means，并用聚类中心初始化码本。这能让初始码向量落在数据密集区域，提高它们在训练早期被选中的机会。

这项设计能降低坍缩风险，但不保证后续训练不再发生坍缩。实际复现时，我还会观察每层码本利用率（被选中过的码字比例）、选择频率分布及不活跃码字数量，检查是否只有少数码向量承担了大部分样本。这些是复现时的诊断建议，不是原文额外报告的实验。

我在另一篇实验笔记中进一步考察了这个问题：提高利用率不一定提高推荐指标，还需要考虑 hard assignment、条件路径和下游预测任务是否对齐。相关实验见：[TIGER 的语义 ID 为什么会坍缩](../tiger-semantic-id-codebook-capacity/)。
::
...
**把一个训练样本的损失接起来看。** 将用户 token 与历史合记为条件 $H$，完整四位 ID 的概率按自回归方式分解为：

$$
P(d_1,d_2,d_3,d_4\mid H)
=\prod_{j=1}^{4}P(d_j\mid H,d_{<j}).
$$

这里 $d_{<j}$ 表示第 $j$ 位之前的正确 token；预测第一位时还没有物品前缀。假设某次训练前向计算给四个正确 token 的概率依次为 0.5、0.8、0.25、0.5，那么可以这样对应起来：

```text
正确标签       d1       d2       d3       d4
对应概率      0.50     0.80     0.25     0.50
负对数损失    0.693    0.223    1.386    0.693
                └────────── 相加 ──────────┘
四位 ID 的损失 = -ln(0.50 × 0.80 × 0.25 × 0.50)
              ≈ 2.996
```

这是说明交叉熵的**假设数值，不是论文实测结果**。概率越低的正确 token，在这个样本中贡献的损失越大。图 4 还画出了结束标记 `<EOS>`；若把它作为第五个预测标签，序列损失还包括预测结束标记的那一项。上式只展开物品 ID 的四位，不能把它误读为图中所有训练位置的完整损失。

[原文第 3.2 节](https://proceedings.neurips.cc/paper_files/paper/2023/file/20dcab0f14046a5c6b02b61da9f13229-Paper-Conference.pdf#page=5)及 [Figure 2b](https://proceedings.neurips.cc/paper_files/paper/2023/file/20dcab0f14046a5c6b02b61da9f13229-Paper-Conference.pdf#page=3)给出了下一物品预测任务与 encoder-decoder 路径；这里的概率分解和数值计算是对这条训练路径的教学展开，并不是原文另列的公式或实验。
...
|---|---:|---:|---:|---:|
| 无显式用户 ID token | 0.04458 | 0.0302 | 0.06479 | 0.0367 |
| 加用户 token | 0.0454 | 0.0321 | 0.0648 | 0.0384 |

两组都输入用户的交互历史。原表的 “No user information” 在这里指去掉额外的用户 ID token，不是把历史条件也删掉。
...
**先看实验规模。** 论文使用 Amazon Reviews 中的三个商品领域，以用户按时间排列的评论记录构成交互序列，并过滤评论少于 5 条的用户。下表是预处理后数据集的概况，序列长度不是展开后的 token 数。

**表 2. 三个主实验数据集的规模与交互序列长度。** 完整转录自[原文附录 C、Table 6](https://proceedings.neurips.cc/paper_files/paper/2023/file/20dcab0f14046a5c6b02b61da9f13229-Paper-Conference.pdf#page=15)；长度单位为物品数。
...
这些实验覆盖的是万级物品库，且序列中位长度为 6。论文训练时最多保留最近 20 个物品，但“最多 20 个”不代表大多数用户都有这么长的历史；因此，用户 token 是否主要补充窗口外偏好，还需要额外证据。
...
- **P5 同样属于生成式方法。** 它生成的是普通物品 ID 的 token 序列，而不是 TIGER 的内容语义码。主表中的 P5 是 TIGER 作者修改物品编号预处理后得到的结果，并非直接照搬 P5 原论文的分数。附录 D 还报告了另一版 P5-ours；两版的差异与结果在主表后的补充中展开。
...
3. **主表比较的是整套系统，不能单独归因于“生成式”或某一种量化器。** 同属生成式方法的 P5 及附录中的 P5-ours 都没有达到 TIGER 的结果，但这不等于差异只来自 ID：内容表示、训练与解码等也可能影响表现。接下来的 ID 对照更适合检验 Semantic ID 的作用。原文 Table 1（本文表 3）也不是多次运行均值表，论文将 TIGER 三次运行的均值与标准误另列在附录 Table 9，尤其不能把 0.0001 这样的显示值差距直接当作显著优势。

::disclosure[补充：主表的 P5 与附录的 P5-ours 有何不同？]

[原文附录 D](https://proceedings.neurips.cc/paper_files/paper/2023/file/20dcab0f14046a5c6b02b61da9f13229-Paper-Conference.pdf#page=15)指出，P5 原实现先按遍历顺序把物品映射为连续整数，再切分训练与测试数据，可能引入编号与序列次序之间的关联。TIGER 作者改用随机整数编号，再进行数据切分；保留 P5 其余代码得到的是 **P5** 这一行。

此外，作者还自行实现了一版 P5，并只在序列推荐任务上训练，记为 **P5-ours**。这组附录结果补充了主表中只有一行 P5 的比较。

**表 4. TIGER 作者报告的两版 P5 结果。** 完整转录自原文 Table 7，按数据集展开；主表采用其中的 P5 行，四项指标均越大越好。

| 数据集 | 版本 | Recall@5 | NDCG@5 | Recall@10 | NDCG@10 |
|---|---|---:|---:|---:|---:|
...
P5-ours 的各项数值都高于 P5。例如 Beauty 的 Recall@10 从 0.0254 提高到 0.048，与 TIGER 的 0.0648 之间的差距明显缩小，但仍未超过 TIGER。这说明实现和任务训练设置值得重视；表 7 没有把这些因素逐项消融，不能把两版差值全归因于其中某一项。
::
...
**表 5. 不同 ID 构造方式的 Recall 对照。** 摘录自原文 Table 2，仅保留 Recall@5 与 Recall@10，原表还报告 NDCG。Sports、Beauty、Toys 分别对应三个主实验数据集；R 表示 Recall，数值越大越好。
...
原文图注明确了统计范围：a 观察全体物品，b 的每个面板只观察第一位编号固定的物品。不过，正文与图注没有给出 Probability 的归一化公式，也没有说明多类别商品如何计数。因此这里用图观察编号与类别的关联，不把某个颜色的条长直接解释为“该类别中有多少百分比选中了某个编号”，也不从图中计算前缀纯度。
...
**表 6. 生成器层数与推荐指标。** 转录自[原文 Table 5，第 10 页](https://proceedings.neurips.cc/paper_files/paper/2023/file/20dcab0f14046a5c6b02b61da9f13229-Paper-Conference.pdf#page=10)。层数指 encoder、decoder 各自的堆叠深度，四项指标均越大越好。原表未单独标注数据集，其 4 层结果与主表 Beauty 配置一致。
...
**原文结果。** 作者将 TIGER 与在语义表示空间中进行近邻搜索的 Semantic_KNN 比较。本文[图 7](#fig-tiger-cold-start)直接展示[原文 Figure 5](https://proceedings.neurips.cc/paper_files/paper/2023/file/20dcab0f14046a5c6b02b61da9f13229-Paper-Conference.pdf#page=8)的两幅曲线；原图图例同时区分了模型与评价对象。

::figure[fig-tiger-cold-start][图 7. Beauty 冷启动实验中的列表长度与新物品配额。a 固定 ε=0.1，改变 K；b 固定 K=10，改变 ε。截取自 TIGER 原文 Figure 5，保留原始坐标、曲线和图例；蓝色为 TIGER，橙色为 Semantic_KNN，实线圆点为 Overall，虚线叉号为 Unseen。]
**a · 返回更多候选，能否提高召回？**

![TIGER 原文 Figure 5a：固定 epsilon 为 0.1，横轴 K、纵轴 Recall@K；同时展示 TIGER 与 Semantic_KNN 的 Overall 和 Unseen 曲线。](media/tiger-figure5a-original.png)

---

**b · 放宽新物品配额，会牺牲整体命中吗？**

![TIGER 原文 Figure 5b：固定 K 为 10，横轴 epsilon、纵轴 Recall@10；TIGER 的 Unseen 曲线上升，而 Overall 曲线下降。](media/tiger-figure5b-original.png)
::

**先区分四条线。** 蓝色代表 TIGER（图例中的 Ours），橙色代表 Semantic_KNN。实线圆点的 **Overall** 衡量整体测试目标的召回，虚线叉号的 **Unseen** 则聚焦目标属于未见物品的测试子集。结合前面的单目标测试设置，可以把它们理解为“所有测试用户中命中了多少”与“目标是新物品的测试用户中命中了多少”。后者不是推荐列表中新物品的占比；抽取 5% 的物品，也不意味着恰好有 5% 的用户属于这个子集。

**a 看候选数量。** 横轴 K 是列表长度，纵轴 Recall@K 是该长度下的命中比例。在固定 $\epsilon=0.1$ 的设置下，TIGER 的整体与新物品召回曲线都高于对应的 Semantic_KNN 曲线。这里比较同一种线型：蓝色实线对橙色实线，蓝色虚线对橙色虚线，而不是把不同评价对象混在一起排名。

**b 看取舍。** 横轴 $\epsilon$ 是新物品比例的上限，纵轴固定为 Recall@10。随着上限放宽，TIGER 的新物品召回上升并逐渐趋于平台，但整体召回下降；Semantic_KNN 的对应曲线在图中保持水平。在图示的 $\epsilon\ge0.1$ 设置下，TIGER 两种口径的曲线仍高于基线，但不能因此把“提高 $\epsilon$”说成整体推荐效果的持续改善。

这组结果说明，TIGER 不仅提供了让新物品进入候选的路径，也展示了**新物品召回与整体命中之间可调的取舍**。原文没有提供这些曲线逐点的数值表，本文保留原图读趋势，不将估读值写成精确实验结果。
...
这里确实还有原文未展开的细节：第 4.3 节只将 Semantic_KNN 描述为语义表示空间中的近邻检索，没有说明如何把用户历史变成查询表示，也没有明确距离函数及候选过滤规则。因此不能自行把它补成“对历史向量取平均，再做余弦搜索”等具体实现。
...
**怎么判断类别是否更丰富？** 原文使用 Entropy@K，即 Top-K 推荐物品的真实类别标签分布之熵。这里的“熵”衡量类别分布有多分散，**不是模型输出 token 概率的熵**。直观地看一份列表：在相同的 $K$ 和类别划分下，越集中于少数类别，熵通常越低；类别覆盖更广、各类别占比更均衡，熵通常越高。

这个直观例子还不等于原文指标的完整计算流程。[原文第 4.3 节及 Table 3](https://proceedings.neurips.cc/paper_files/paper/2023/file/20dcab0f14046a5c6b02b61da9f13229-Paper-Conference.pdf#page=9)没有说明跨用户是先分别计算再平均，还是汇总类别后计算，也没有明确多标签商品、类别层级及对数底数的处理。下面保留原表数值和同一列内的温度变化趋势，不另行指定这些实现细节或熵的单位。
...
**表 7. Beauty 数据集上，不同解码温度下推荐列表的类别熵。** Entropy@10、@20 和 @50 分别考察前 10、20 和 50 个推荐物品；数值越高表示类别分布越分散，不表示推荐越准确。数据来源为上文所引原文 Table 3。
...
**表 8. 扩大物品语料后，Beauty 推荐指标的变化。** 数值转录自[原文 Table 10，第 17 页](https://proceedings.neurips.cc/paper_files/paper/2023/file/20dcab0f14046a5c6b02b61da9f13229-Paper-Conference.pdf#page=17)。四项指标均越大越好；带星号的数值保留原表异常值，不据此计算相对变化。
...
这个差别可以沿[图 8](#fig-tiger-index-map)的查询路径理解：传统双塔模型先为用户与物品生成向量，再由外部向量索引寻找匹配；TIGER 则由 Transformer 根据历史直接生成候选地址。模型参数学到的是“什么上下文更可能对应什么 ID”，不是保存了一张可以直接浏览的物品记录表。
...
模型可以生成一个语法上完整、但数据库中不存在的 Semantic ID。[原文第 4.5 节](https://proceedings.neurips.cc/paper_files/paper/2023/file/20dcab0f14046a5c6b02b61da9f13229-Paper-Conference.pdf#page=10)报告，Top-10 的无效 ID 比例约为 0.1%–1.6%；[附录 Figure 6](https://proceedings.neurips.cc/paper_files/paper/2023/file/20dcab0f14046a5c6b02b61da9f13229-Paper-Conference.pdf#page=16)分别绘出 Sports、Beauty、Toys 随 K 变化的曲线，Top-20 的比例范围约为 0.3%–6%。这里统计的是 beam search 生成的 Top-K ID 中无法对应到物品的比例，不是物品库中无效地址的比例，也不是过滤后列表的错误率。

这只是实验现象，并不是理论保证。

论文建议扩大 [beam size](term:beam-search)，先生成更多候选，再过滤无效 ID，以补足需要的有效结果；还提出未来可以用 prefix matching 将无效 ID 映射到共享有效前缀的物品。扩大 beam 是一种补救策略，并不意味着任意有限的 beam 宽度都必然能补足 K 个有效物品。

作为进一步的实现思路，可以维护由物品库中有效 ID 构成的 trie，在每一步解码时只允许有效后继，并且只在完整有效 ID 处结束。这样能约束输出地址的合法性，但会重新引入外部前缀结构；这是本文提出的方案，不是 TIGER 已报告的实验。
...
但生成式检索没有消灭检索成本，只是重新分配了成本。TIGER 需要逐 token 自回归解码，并通过 [beam search](term:beam-search) 获取 Top-K。同一步的多个候选分支可以批量计算，但下一步仍依赖当前前缀；因此差别不应简化成“ANN 能并行、生成完全不能并行”。[原文附录 E](https://proceedings.neurips.cc/paper_files/paper/2023/file/20dcab0f14046a5c6b02b61da9f13229-Paper-Conference.pdf#page=16)也明确承认自回归 beam search 可能带来更高推理成本，而未给出端到端延迟或吞吐对比。
