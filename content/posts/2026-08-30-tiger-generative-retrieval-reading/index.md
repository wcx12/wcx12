...
第一种是**重构意义上的粗到细**。第一层先从自己的码本中选择距离潜在向量最近的向量，后续层继续近似剩余 residual。这里的“细”指继续修正量化误差，并不预先对应某一种语义属性；残差大小也不是对每个样本都严格单调下降的结构保证。
...
这个结论不会由残差量化自动产生。即使逐层重构确实改善，也只能说明后续码向量补充了近似信息，不能直接推出每一位都对应人类标签体系里的一级分类。

**原文展示了什么？** [第 4.2 节的定性实验及原文 Figure 4](https://proceedings.neurips.cc/paper_files/paper/2023/file/20dcab0f14046a5c6b02b61da9f13229-Paper-Conference.pdf#page=6)在 Amazon Beauty 上把三层码本大小设成 4、16、256，观察前两位编号与商品类别的对应关系。原图将第一、第二位写为 $c_1,c_2$；按本文从 0 开始的记号，它们对应 $c_0,c_1$。图中可以看到类别聚集，作者据此将这些 ID 解释为具有语义层次。

**我对这个解释有什么疑问？** 首先，可视化采用的 4、16、256 与主实验的 256、256、256 不同。小码本限制了可用前缀数量，但不能仅凭容量判断实际共享率或类别纯度。因此我希望看到主配置下的类别分布，以及前缀纯度、类内距离和不同随机种子的结果，而不只依据一组小码本可视化作推广。

其次，[原文的内容特征](https://proceedings.neurips.cc/paper_files/paper/2023/file/20dcab0f14046a5c6b02b61da9f13229-Paper-Conference.pdf#page=5)本来就包含 category。量化后的类别聚集，可能部分继承了输入文本及 Sentence-T5 表示中的类别信息。这个设计本身不等于训练测试泄漏，但该图无法独自区分“保留了已有类别结构”和“RQ-VAE 额外学出了层次”。去掉类别字段的对照，以及量化前后在同一指标下的比较，会帮助回答这个问题。
...
ID 对照之后，还需要看结果是否依赖某个特定的生成器配置。前文采用 encoder 和 decoder 各 4 层的设置；论文还比较了两侧各 3、4、5 层的配置。
...
我认为这里需要区分两种证据：原文 Table 3 和 Table 4 展示了提高温度后的类别变化，但没有分别比较“只调整第一位”和“只调整后续位”的结果。要判断能否按层级控制推荐，我希望固定其他生成条件，分别改变不同位置的采样方式，再检查变化究竟发生在类别之间，还是同类商品之间。这也与前面对[“从粗到细”](#讨论-从粗到细-究竟是什么意思)的疑问相连。
::
...
这个差别可以沿[图 6](#fig-tiger-index-map)的查询路径理解：传统双塔模型先为用户与物品生成向量，再由外部向量索引寻找匹配；TIGER 则由 Transformer 根据历史直接生成候选地址。模型参数学到的是“什么上下文更可能对应什么 ID”，不是保存了一张可以直接浏览的物品记录表。
