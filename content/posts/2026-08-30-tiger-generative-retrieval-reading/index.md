...
论文第 4.1 节把 TIGER 和 GRU4Rec、Caser、HGN、SASRec、BERT4Rec、FDSA、S3-Rec、P5 等 sequential recommendation baseline 放在一起比较，指标是 Recall@5/10 和 NDCG@5/10。

下表列出每个数据集上 TIGER 与对应最强 baseline 的对比；“最强 baseline”按每个指标分别取原表中除 TIGER 外的最好结果。
...
但这个主结果本身还不能告诉我们“为什么赢”。可能是因为 Semantic ID 让相似 item 共享 token，也可能是因为内容 embedding 帮助低频 item，也可能只是某些解码和训练细节更合适。这些可能性需要通过 ID 对照、配置消融和能力实验分别检验，不能仅凭总指标提升确定原因。
